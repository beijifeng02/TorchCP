...
from libs.cfgs import cfg
from commons.utils import set_seed, Smooth_Adv_ImageNet, calculate_accuracy_smooth
...
checkpoint = torch.load('pretrained_model/checkpoint.pth.tar', map_location=device)
...
if not os.path.exists(cfg["directory"]["value"]):
    ...
    with open(cfg["directory"]["value"] + "/data.pickle", 'rb') as f:
